 ...
 */
export function ParticleTextBase(props) {
  const {
    text = 'KLYRO',
    colors = ['#FFFFFF', '#22d3ee', '#6366f1'],
    mode = 'onHover',
    replay = true,
    position = 'above',
    particleSize = 10,
  ...
  const mcRadius = typeof mouseRadius === 'number' ? mouseRadius : 55;
  const mcForce = typeof mouseForce === 'number' ? mouseForce : 25;
  ...
  useEffect(() => {
    ...
    hiddenRef.current = mode === 'onHover';
    formValRef.current = mode === 'onHover' ? 0 : 1;
...
export default function ParticleText(props) {
  return <ParticleTextBase {...originkitPresetProps} {...props} />;
}
